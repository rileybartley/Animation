...
{
	//Tcl_CreateCommand(interp, "part1", Command, (ClientData)NULL,
	//	(Tcl_CmdDeleteProc*)NULL);
	//Tcl_CreateCommand(interp, "part2", Command, (ClientData)NULL,
	//	(Tcl_CmdDeleteProc*)NULL);
...
int animTcl::Command(ClientData clientData, Tcl_Interp *interp, int argc, myCONST_SPEC char **argv)
{
	if (argc < 2 ) {
		/*
		if (strcmp(argv[0], "part1") == 0) {
			//start part 1 stuff
			BaseSystem* hermiteSystem =
				GlobalResourceManager::use()->getSystem("hermite");
			
		}
		else if (strcmp(argv[0], "part2") == 0) {
