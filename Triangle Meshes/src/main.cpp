...
double ray_triangle_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &a, const Vector3d &b, const Vector3d &c, Vector3d &p, Vector3d &N)
{
    ...
    Vector3d pgram_u = b-a; //v2-v1
    Vector3d pgram_v = c-a; //v3-v1

    MatrixXd A_matrix(3,3);

    A_matrix << pgram_u, pgram_v, -ray_direction;
    
    Vector3d B_vector = ray_origin-a;
    
    MatrixXd A_inverse(3,3);
    A_inverse = A_matrix.inverse();
    Vector3d X_vector = A_inverse*B_vector;

    if (X_vector[0] <= 0 && X_vector[1] <= 0 && X_vector[0]+X_vector[1] <= 1 && X_vector[2] > 0)
    {
        // TODO: The ray hit the triangle, compute the exact intersection
        // point
        p = ray_origin + (X_vector[2] * ray_direction); // e+td
...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &a, const Vector3d &b, const Vector3d &c, Vector3d &p, Vector3d &N, int index)
{
    Vector3d pgram_u = b-a; //v2-v1
    Vector3d pgram_v = c-a; //v3-v1
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &a, const Vector3d &b, const Vector3d &c, Vector3d &p, Vector3d &N, int index)
{
...
bool find_nearest_object(const Vector3d &ray_origin, const Vector3d &ray_direction, Vector3d &p, Vector3d &N)
{
    ...
    for (int i = 0; i < centeroids.size(); ++i)
    {
        ...
        const double t = ray_triangle_intersection(ray_origin, ray_direction, i, tmp_p, tmp_N);
